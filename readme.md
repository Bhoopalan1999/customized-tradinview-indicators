# Pivot Indicator Forked & Fib EMAs

This TradingView script combines features from the Pivot Indicator and Fibonacci Exponential Moving Averages (EMAs). It provides an extensive set of options for pivot type, timeframe, and display preferences.

## Description

This script merges the Pivot Indicator functionalities with Fibonacci EMA calculations to offer comprehensive insights into pivot points and moving averages. The script supports various pivot types (Traditional, Fibonacci, Woodie, Classic, Demark, Camarilla), enabling users to tailor the indicator to their preferred trading strategies.

## Configuration Options

### Type
- Select the pivot type among Traditional, Fibonacci, Woodie, Classic, Demark, and Camarilla.

### Pivots Timeframe
- Choose the desired timeframe for pivot calculations, with options ranging from Auto, Daily, Weekly, Monthly, Quarterly, Yearly, to longer-term periods like Biyearly, Triyearly, Quinquennially, and Decennially.

### Number of Pivots Back
- Adjust the number of previous pivots considered in the calculation.

### Use Daily-based Values
- Toggle to use either daily-based values for pivot calculations or intraday data, especially relevant when working on intraday charts or displaying extended hours.

### Display Preferences
- Configure options to show labels, prices, label positions, and line width for better visualization of pivot points.

### EMAs
- The script also includes three Exponential Moving Averages (EMAs) - 13, 34, and 55 - visualized on the chart.

## Usage

The script calculates pivot points based on the chosen pivot type and timeframe, offering insights into potential support and resistance levels. Additionally, it presents three EMAs to aid in trend identification.

## Notes

- Ensure sufficient data is available to calculate pivot points, especially for intraday data or extended hours displayed on charts.

Feel free to customize the parameters based on your trading strategy and chart analysis needs.

For further details on the calculation methodology and customizations, refer to the code comments and the script itself for a more in-depth understanding.

This script aims to be a comprehensive tool for pivot point analysis, supporting various pivot types and offering EMAs for trend identification.
...
// © imbhoopalan
// Manual versioning control done through Github: 

//@version=5

indicator("Pivot Indicator Forked & Fib EMAs", overlay=true, max_lines_count=500, max_labels_count=500)
//@version=6

////////////////////////////////////////////////////////////////////////
//The Indicator is combination of both pivot indicator and FIb EMAs/////
////////////////////////////////////////////////////////////////////////
...
p_color = input.color(#6699ff, group="levels")
p_show = input.bool(true, "", inline="P", group="levels")
var r1 = array.new_float()
var s1 = array.new_float()
s1_color = input.color(#00ff00, group="levels")
s1_show = input.bool(true, "", inline="S1/R1", group="levels")
r1_color = input.color(  #ff5c33, group="levels")
r1_show = input.bool(true, "", inline="S1/R1", group="levels")
var r2 = array.new_float()
var s2 = array.new_float()
s2_color = input.color( #33cc33, group="levels")
s2_show = input.bool(true, "", inline="S2/R2", group="levels")
r2_color = input.color(  #e62e00, group="levels")
r2_show = input.bool(true, "", inline="S2/R2", group="levels")
var r3 = array.new_float()
var s3 = array.new_float()
s3_color = input.color( #009900, group="levels")
s3_show = input.bool(false, "", inline="S3/R3", group="levels")
r3_color = input.color( #cc2900, group="levels")
r3_show = input.bool(false, "", inline="S3/R3", group="levels")
...
//EMAs
smallEMA = ta.ema(close, 13)
mediumEMA = ta.ema(close, 34)
bigEMA = ta.ema(close, 55)

plot(smallEMA, color=#006400)
plot(mediumEMA, color=#FFA500)
plot(bigEMA, color=#800000)
